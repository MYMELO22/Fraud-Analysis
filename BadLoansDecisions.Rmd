...
library(dplyr)
library(readr)
loanData = read_rds("/Users/manaka/Desktop/LoanData.rds")
...
gini_left =getGini(8015,1000)
gini_right =getGini(17850,2227)
left = 1000+8015
right =17850+2227

gini_root - (left/total * gini_left) - (right/total * gini_right)
```
...
```
Gini Gain of feature X is 4.622219e-12, nearly 0. There is no value of adding feature X. 
...
# It means that there is no variable and split that could produce more homogenous subgroups than the original group.
```
...
```{r}
### put default data into a variable called loanTrain_default
loanTrain_default = loanData[1 ==loanData$isLoanDefault, ]
###put non-default data into a variable called loanTrain_nodefault
loanTrain_nodefault = loanData[0 ==loanData$isLoanDefault, ]
###generate an indice where 1/3 of the cases are default and 2/3 are are not default.
...
###only include 2/3 non-default into a variable loanTrain_nodefault
loanTrain_nodefault = loanTrain_nodefault[loanTestIndices,]
### bind loanTrain_default with loanTrain_nodefault by rows
...
### This section doesn't require code but feel free to reprint any critical values.
```
uniform = TRUE is uniform vertical spacing of the nodes. A large plot may be less cluttered when fitting onto a page. 
...
```{r}
tree_defaults_balanced_parms = rpart(isLoanDefault ~ ., method="class", data = loanTrain_balanced, control = rpart.control(cp = 0.001),parms = list(prior = c(0.7, 0.3))) 
...
```{r}
tree_defaults_balanced_parms_lossmatrix = rpart(isLoanDefault ~ ., method="class", data = loanTrain_balanced, control = rpart.control(cp = 0.001),parms = list(loss = matrix(c(0, 2, 1, 0), ncol = 2), prior = c(0.67, 0.33))) 

tree_defaults_balanced_parms_lossmatrix = rpart(isLoanDefault ~ ., method="class", data = loanTrain_balanced, control = rpart.control(cp = 0.001),parms = list(loss = matrix(c(0, 6, 8, 0), ncol=2))) 
